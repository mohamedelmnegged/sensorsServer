...
def on_connect(client, userdata, flags, rc):
    ...
    client.subscribe("/esp8266/fire")
    # subscribe for each rout
    # firebase connection
    cred = credentials.Certificate("graduation-project-4b1ce-firebase-adminsdk-d9l8z-0575b351ad.json")
    firebase_admin.initialize_app(cred,
                                  {'databaseURL': 'https://graduation-project-4b1ce-default-rtdb.firebaseio.com/'})
...
def on_message(client, userdata, message):
    ...
    if message.topic == "/esp8266/temperature":
        ...
        temp=str(message.payload)[2:-1]
        # convert temp to double
        # save to firbase
        ref = db.reference('Sensors/')
        ref.child('Temperature').set(float(temp))
        socketio.emit('dht_temperature', {'data':temp})
    if message.topic == "/esp8266/humidity":
        print("humidity update")
        temp = str(message.payload)[2:-1]
        ref = db.reference('Sensors/')
        ref.child('Humidity').set(float(temp))
        socketio.emit('dht_humidity', {'data': str(message.payload)[2:-1]})
    if message.topic == "/esp8266/gas":
        print("gas update")
        temp = str(message.payload)[2:-1]
        ref = db.reference('Sensors/')
        ref.child('Gas').set(float(temp))
        socketio.emit('dht_gas', {'data': str(message.payload)[2:-1]})
    if message.topic == "/esp8266/fire":
        print("fire update")
        temp = str(message.payload)[2:-1]
        ref = db.reference('Sensors/')
        ref.child('Fire').set(float(temp))
        socketio.emit('dht_fire', {'data': str(message.payload)[2:-1]})
